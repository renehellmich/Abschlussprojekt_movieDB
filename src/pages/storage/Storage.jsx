...
import './storage.css'
import "../../components/searchShowList/searchShowList.css"

const Storage = () => {

    const { storageMovie, setStorageMovie, setMovieID, setBackPath } = useContext(mainContext)

    const navigate = useNavigate()

    const deleteAllMovie = (event) => {
        event.preventDefault()
        localStorage.removeItem('storageMovie')
        setStorageMovie([])
    }
    ...
    const unSaveMovie = (id) => {
        console.log(id);
        const movies = JSON.parse(localStorage.getItem('storageMovie'))
        const updatedMovies = movies.filter(movie => movie.id != id)

        localStorage.setItem('storageMovie', JSON.stringify(updatedMovies))
        setStorageMovie(updatedMovies)
    }

    const storage = JSON.parse(localStorage.getItem('storageMovie'))

    return (
        <>
            <main className='mainStorageMovies'>
                <h1 >My saved movies.</h1>

                {
                    storage && storage.length > 0 ? storage.map((movie, index) => {
                        return (
                            <div key={index} className='movieCard'>
                                <div className="containerFoto" onClick={() => goToDetails(movie.id)}>
                                    <img className="showListImg" src={`https://image.tmdb.org/t/p/w342${movie.poster_path}`} alt={movie.title}></img>
                                </div>
                                <div className="containerInfo" >
                                    <p className="movieTitle" onClick={() => goToDetails(movie.id)}>
                                        {movie.title.split()}
                                    </p>
                                    <svg className="bookmarkIcon" xmlns="http://www.w3.org/2000/svg" width="19" height="23" viewBox="0 0 19 23" fill="none" onClick={() => unSaveMovie(movie.id)}>
                                    ...
                                    <div className="movieInfos" onClick={() => goToDetails(movie.id)}>
                        ...
                        One ought to consider adding films prior to perusing through one's favorites, hmm???</p>
                }
                <div className='deleteButtonContainer'>
                    <button
                        onClick={deleteAllMovie}
                        className='buttonRed'
                    >Delete List</button>
                </div>
            </main>
